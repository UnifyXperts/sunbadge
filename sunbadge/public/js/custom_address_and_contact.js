...
frappe.provide("sunbadge.contacts");

if (frappe.templates.contact_list) {
	frappe.templates.contact_list = frappe.templates.contact_list.replace(
		`{% if (contact.designation){ %}`,
		`{% if (contact.custom_is_shipping_contact) { %}
			&nbsp;&#183;&nbsp;
			<span class="text-muted">{%= __("Shipping Contact") %}</span>
		{% } %}
...
$.extend(sunbadge.contacts, {
	...
	render_address_and_contact: function (frm) {
        console.log("🔥 SUNBADGE CUSTOM RENDERER RUNNING");
...
function new_record(doctype, frm) {
	...
	if (frappe.boot.enable_address_autocompletion === 1 && doctype === "Address") {
        console.log("Address Autocomplete Dialog");
